...
def get_negative_augmentations(height, width, enable_rotate90=True):
    aug_list = [
        A.CropAndPad(
            percent=(-0.25, 0.0), pad_mode=cv2.BORDER_CONSTANT, pad_cval=0, p=0.4
        ),  # Giảm aggressive nhẹ
        A.CoarseDropout(
        ...
        A.ShiftScaleRotate(shift_limit=0.2, scale_limit=0.3, rotate_limit=60, p=0.5),
        A.ElasticTransform(alpha=1.5, sigma=40, p=0.2),
        A.GridDistortion(num_steps=5, distort_limit=0.3, p=0.25),
        A.OpticalDistortion(distort_limit=0.4, shift_limit=0.4, p=0.25),
        ...
        A.Equalize(p=0.4),
        A.HueSaturationValue(
            hue_shift_limit=25, sat_shift_limit=35, val_shift_limit=25, p=0.4
        ),
        ...
        A.UnsharpMask(
            blur_limit=(5, 9), sigma_limit=(0.0, 4.0), alpha=(0.3, 0.6), p=0.3
        ),
        A.OneOf(
            [
                A.GaussNoise(var_limit=(20, 100), p=1.0),
                A.ISONoise(color_shift=(0.02, 0.12), intensity=(0.2, 0.9), p=1.0),
        ...
        A.ColorJitter(brightness=0.25, contrast=0.25, saturation=0.25, hue=0.15, p=0.3),
        A.ToGray(p=0.1),
        A.ToSepia(p=0.1),
        A.RandomSunFlare(
            flare_roi=(0, 0, 1, 0.5), angle_lower=0, p=0.2
        ),  # Thêm sun flare
        A.RandomShadow(
    ...
    if enable_rotate90:
        aug_list.insert(4, A.RandomRotate90(p=0.6))
    return aug_list
...
def get_train_augmentation_negative(height, width, enable_rotate90=True):
    big_h = int(height * 1.25)
    big_w = int(width * 1.25)
    aug_list = [A.Resize(big_h, big_w)]
